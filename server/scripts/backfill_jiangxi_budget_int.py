#!/usr/bin/env python3
"""Backfill jiangxi.budget from content as integer yuan (no decimal point).

Examples:
  预算金额：2,200,000 元  -> 2200000元
  预算金额：132.00 万元   -> 1320000元
  list budget 27          -> 270000元 (万元换算，仅正文无金额时)
"""
...
def main() -> None:
    ...
    with conn:
        with conn.cursor() as cur:
            cur.execute("SELECT id, budget, content_text FROM jiangxi")
            rows = cur.fetchall()
        ...
        for row in rows:
            ...
            if new == old:
                continue
            # allow clearing zero budgets to empty string
            if not new and src != "clear_zero":
                continue
        ...
        with conn.cursor() as cur:
            if updates:
                cur.executemany("UPDATE jiangxi SET budget=%s WHERE id=%s", updates)
        conn.commit()
